...
public partial class MainWindow : ui.FluentWindow
{
    ...
    public MainWindow()
    {
        ...
        if (AppTitleBar != null) AppTitleBar.Title = $"RUST+ DESKTOP - PANDA EDITION v{AppInfo.VersionRaw}";
        this.Title = $"RUST+ DESKTOP - PANDA EDITION v{AppInfo.VersionRaw}";
    ...
    private static readonly Dictionary<int, string> sDynIconByType = new()
{
    ...
    private void MainWindow_Closed(object? sender, EventArgs e)
    {
        try
        {
            if (_pairing?.IsRunning == true)
                _pairing.StopAsync().GetAwaiter().GetResult();
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"Pairing listener shutdown failed: {ex.Message}");
        }
...
private sealed record MarkerRef(System.Windows.Shapes.Ellipse Dot, double U_DIP, double V_DIP, double Radius);
    ...
    {
        line = SensitiveLogRedactor.Redact(line);
    ...
    private void BtnDonate_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            {
                FileName = "https://github.com/LazyPandax/rust-",
    ...
    private static string PairingConfigPath => FcmConfigStore.PlainConfigPath;
    ...
    private async Task<bool> ResetPairingConfigAsync(bool stopListenerFirst = true)
    {
        try
        {
            ...
            if (File.Exists(PairingConfigPath) || File.Exists(FcmConfigStore.ProtectedConfigPath))
            {
                FcmConfigStore.DeleteAll();
    ...
    private static void RevealDownloadedInstaller(string path)
    {
    ...
    private async Task<bool> VerifyAndRevealDownloadedInstallerAsync(string installerPath, string? expectedSha256)
    {
        try
        {
            var actualSha256 = await ComputeFileSha256Async(installerPath);
            AppendLog($"Downloaded installer: {installerPath}");
            AppendLog($"Installer SHA256: {actualSha256}");

            if (!string.IsNullOrWhiteSpace(expectedSha256) &&
                !string.Equals(actualSha256, expectedSha256, StringComparison.OrdinalIgnoreCase))
            {
                AppendLog("Update digest mismatch. Installer was not started.");
                System.Windows.MessageBox.Show(
                    $"Downloaded installer hash does not match the GitHub release digest.\n\nPath: {installerPath}\nExpected SHA256: {expectedSha256}\nActual SHA256:   {actualSha256}",
        ...
        {
            AppendLog("Could not verify downloaded installer: " + ex.Message);
            System.Windows.MessageBox.Show("Could not verify downloaded installer.\n" + ex.Message,
