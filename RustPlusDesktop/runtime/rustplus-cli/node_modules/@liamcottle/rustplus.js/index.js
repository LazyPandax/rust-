...
function getPreferredBrowserPath() {
    const envCandidates = [
        process.env.RUSTPLUSDESK_BROWSER_PATH,
        process.env.CHROME_PATH,
        process.env.PUPPETEER_EXECUTABLE_PATH,
    ];

    if (process.platform !== 'win32') {
        return firstExisting(envCandidates);
    }
    ...
    const relativePaths = [
        ...
        path.join('Vivaldi', 'Application', 'vivaldi.exe'),
        path.join('Programs', 'Opera GX', 'opera.exe'),
        path.join('Programs', 'Opera', 'opera.exe'),
        path.join('Programs', 'Opera GX', 'launcher.exe'),
        path.join('Programs', 'Opera', 'launcher.exe'),
        path.join('Opera', 'opera.exe'),
        path.join('Opera GX', 'opera.exe'),
    ];
...
function getPairingProfileDir() {
    return path.join(os.tmpdir(), 'rustplusdesk-pairing-profile');
}
...
async function linkSteamWithRustPlus() {
    return new Promise((resolve, reject) => {
        ...
        server = app.listen(port, async () => {
             ...
             */
            const browserPath = getPreferredBrowserPath();
            const launchOptions = {
                ...
                chromeFlags: [
                    ...
                    `--user-data-dir=${getPairingProfileDir()}`, // create a new browser profile for pair.js
                ],
            ...
            if (browserPath) {
                ...
                console.log("Launching Chromium browser for Rust+ pairing: " + browserPath);
            }

            await ChromeLauncher.launch(launchOptions).catch((error) => {
                console.log(error);
                console.log("pair.js failed to launch a Chromium browser. Install Microsoft Edge, Google Chrome, Brave, Opera GX, or set RUSTPLUSDESK_BROWSER_PATH.");
                process.exit(1);
            });
        });
    });
...
async function fcmRegister(options) {
    ...
    // tell user to link steam with rust+ through a Chromium-based browser
    console.log("A Chromium browser is launching so you can link your Steam account with Rust+");
