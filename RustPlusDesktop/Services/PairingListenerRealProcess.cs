...
namespace RustPlusDesk.Services
{
    ...
    new(@"value:\s*(?:'|`)(?<json>\{.*?\})(?:'|`)", RegexOptions.Compiled | RegexOptions.Singleline);
        ...
        public PairingListenerRealProcess(Action<string> log)
            => _log = s => log(SensitiveLogRedactor.Redact(s));
        ...
        public bool IsConfigured => FcmConfigStore.IsConfigured;



        private string ConfigPath => FcmConfigStore.PlainConfigPath;
    ...
    bool waitForExit = true, bool redirect = true, CancellationToken token = default)
        ...
        private static string HumanizeCli(string s)
        {
            ...
            if ((l.Contains("status code 503") || l.Contains("service unavailable")) &&
                (l.Contains("expo") || l.Contains("exp.host") || l.Contains("getexpopushtoken")))
    ...
    params (string key, string value)[] env)
        ...
        public void EnrichFcmConfig(DateTime issuedAt, DateTime expiresAt, string? steamId)
        {
            try
            {
                ...
                var enrichedJson = Encoding.UTF8.GetString(ms.ToArray());
                FcmConfigStore.SaveConfigJson(enrichedJson, keepPlaintext: true);
